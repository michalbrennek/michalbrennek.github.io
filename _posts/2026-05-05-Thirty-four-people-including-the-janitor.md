...
A **Population Count** instruction -- which counted the number of one-bits in a sixty-bit word -- was also part of the 6600's instruction set, included at the explicit request of the National Security Agency for cryptanalytic work. Population Count was not a separate functional unit; it was implemented using the divide unit's hardware, taking roughly sixty-eight clock cycles to complete via divide-unit microcode.[^4]
...
The departure was, by the standards of corporate technical-leadership transitions, exceptionally amicable. Cray and Norris had worked together for fifteen years; they parted on terms that included Norris's personal blessing of Cray's planned new company, a CDC equity injection of approximately two hundred and fifty thousand dollars in seed capital, and an explicit non-compete-by-mutual-restraint that would keep CDC and Cray's new company out of each other's principal product segments for several years. CDC retained a small equity stake in the new company, and Norris would later describe the spinoff as a healthy thing for both organisations.
